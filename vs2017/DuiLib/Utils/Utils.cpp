...
namespace DuiLib {
	...
	CDuiString::CDuiString (LPCTSTR lpsz, int nLen) : m_pstr (m_szBuffer) {
		ASSERT (!::IsBadStringPtr (lpsz, (UINT_PTR) -1) || lpsz == nullptr);
		m_szBuffer[0] = '\0';
		Assign (lpsz, nLen);
	}
	...
	const CDuiString& CDuiString::operator=(LPCSTR lpStr) {
		if (lpStr) {
			...
			if (pwstr != nullptr) ::MultiByteToWideChar (::GetACP (), 0, lpStr, -1, pwstr, cchStr);
			Assign (pwstr);
		} else {
			Empty ();
		}
	...
	const CDuiString& CDuiString::operator=(LPCWSTR lpwStr) {
		if (lpwStr) {
			...
			if (pstr != nullptr) ::WideCharToMultiByte (::GetACP (), 0, lpwStr, -1, pstr, cchStr, nullptr, nullptr);
			Assign (pstr);
		} else {
			Empty ();
		}
	...
	int CDuiString::InnerFormat (LPCTSTR pstrFormat, va_list Args) {
		...
		while (TRUE) {
			...
			if ((szBuffer = (TCHAR*) realloc (szBuffer, size)) != nullptr) {
				ZeroMemory (szBuffer, size);
			} else {
				break;
			}
		...
		int nLen, totalLen;
		TCHAR *szBuffer;
		nLen = _vsntprintf (nullptr, 0, pstrFormat, Args);
